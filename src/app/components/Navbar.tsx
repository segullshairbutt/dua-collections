              ...
              <Link href="/">
                <div className="avatar">
                  <div className="w-16 rounded">
                    <h1 className="text-3xl text-white font-bold">Dua Collection</h1>
                  </div>
                </div>
              </Link>
            </div>
            ...
            <ul className="items-center justify-center space-y-8 md:flex md:space-x-6 md:space-y-0">
              {menu.map(({ name, url }, index) => {
                ...
                return (
                  <li
                    key={index}
                    className={`block text-sm font-semibold text-gray-400 hover:bg-indigo-50 active:bg-indigo-100 hover:text-indigo-600 rounded ${
                      isActive && 'text-indigo-50'
                    }`}
                  >
                    <Link
                      className="block w-full p-4"
                      onClick={() => setNavbar(prev => (prev ? false : prev))}
                      href={url}
                    >
                      {name}
                    </Link>
