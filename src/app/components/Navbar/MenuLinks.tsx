        ...
        <ul className={ulClassName}>
            {
                menu.map(({ name, url }, inx) => (
                    <NavLink
                        ...
                        name={name}
                        toggle={toggle}
                        isActive={queryParm === url.query?.tag}
                    />
                ))
            }
        </ul>
...
const NavLink = ({ name, toggle, url, isActive }: NavLinkProps) => {
    return (
        <li className={`block text-sm font-semibold text-gray-400 hover:bg-indigo-50 active:bg-indigo-100 hover:text-indigo-600 rounded ${isActive && 'text-indigo-50'}`}>
